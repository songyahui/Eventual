...
\begin{abstract}
This paper presents \emph{Pledge}, a Haskell library that extends the classical
...
operation can depend on its own return value, e.g., requiring that a returned file handle be eventually closed.
...
using a subtraction operator. Preconditions and postconditions compose 
in the Hoare-rule style.
The design is parameterised over a \lstinline{Composable} algebra -- 
requiring concatenation~($\diamond$), conjunction~($\wedge$), subtraction~($\setminus$),
with identity elements \lstinline{empty} (for concatenation) and \lstinline{universe} (for conjunction).
Here presents four instances of the framework: trace protocols via regular expressions ($\RE$);
...
All four instances share the same monadic bind formula, demonstrating that
