...
complement, and cover the common quantitative patterns.
...
the accumulated residual is satisfiable or entailed.
...
\paragraph{Precondition and the monad-law status of \lstinline{pre}.}
The \lstinline{pre} field satisfies the Hoare-rule property
...
It is worth being precise about what this means for the \lstinline{Monad}
instance.
The triple $(\ret, \post, \widehat{\fut})$ satisfies the three monad laws
exactly, as proved above.
The \lstinline{pre} field is an \emph{independent annotation}: it is not
part of the monad's return value or sequencing behaviour, but rather a
separate summary accumulated alongside it, analogous to a writer-monad log.
Consequently, \lstinline{Pledge} is a lawful \lstinline{Monad} when
equality is taken to mean equality of $(\ret, \post, \widehat{\fut})$;
the \lstinline{pre} field participates in a separate \emph{contract
checking} layer that deliberately deviates from left-identity when a
precondition violation is detected.
In practice, a programmer inspects \lstinline{pre} and
\lstinline{evalFuture} after constructing the specification term,
and both fields equal \lstinline{universe} if and only if the
composed program is contract-correct.
