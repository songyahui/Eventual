...
Value-dependence is essential: it lets an obligation name the particular
resource handle the operation returns --- requiring, for instance, that
the \emph{specific} context produced by \lstinline{tlsHandshake} must
eventually have \lstinline{bye} called on it.
...
Annotations are per-operation; the monad handles all propagation and
...
All four instances share a single monadic bind formula, showing that
diverse obligation domains --- trace-based, arithmetic, quantitative,
and heap-ownership --- are uniformly expressible within one algebraic
interface.
\end{abstract}
...
can name the actual resource handle produced at runtime rather than one
fixed at specification time: \lstinline{tlsHandshake} can declare that
\emph{whichever} \lstinline{Context} it returns must eventually receive
\lstinline{bye}.
Monadic bind partially discharges the first computation's future
condition against the continuation's postcondition via
\lstinline{subtraction}; the residual is conjoined with the
...
That all four share a single monadic bind formula is not a coincidence:
trace-based, arithmetic, quantitative, and heap-ownership obligations
are structurally identical at the level of the \lstinline{Composable}
algebra.
...
combines simultaneous constraints; \lstinline{subtraction} computes the
residual obligation (the Brzozowski quotient for the RE instance).
...
\paragraph{Why \lstinline{future = \\_ ->} in the bind result.}
The implementation writes \lstinline{future = \\_ -> ...} in the result of
\lstinline{>>=}, which may appear to discard the data-dependent argument
and suggest that value-dependence is unnecessary.
This is not the case.
Data-dependence is \emph{essential} and is already exploited: the
expression $\fut(e)(\ret(e))$ evaluates $e$'s future at $e$'s concrete
return value, and $\fut(e_2)(\ret(e_2))$ evaluates $e_2$'s future at
$e_2$'s concrete return value.
After both evaluations the combined obligation is a fully-instantiated
\lstinline{eff} value, so the resulting \lstinline{future} field of the
composed \lstinline{Pledge} happens to be a constant function---it no
longer needs to inspect the outer return value because the inner return
values have already been consumed.
The \lstinline{\_} simply reflects this: all value-dependent
instantiation has already been performed inside the lambda body.
...
The structure mirrors \lstinline{>>=}: preconditions and postconditions
...
\emph{depend} on the return value of $e_f$---the function inside
$e_f$ and the argument inside $e_x$ are constructed independently.
In contrast, \lstinline{>>=} passes $\ret(e)$ directly into $f$,
enabling the data-dependent future condition exemplified by
\lstinline{tlsHandshake} and \lstinline{takeMVar}.

\paragraph{When to use \lstinline{<*>} vs \lstinline{>>=}.}
Use \lstinline{<*>} when the right-hand computation is independent of
the left-hand return value (e.g.\ two stateless checks composed
together).
Use \lstinline{>>=} (or \lstinline{do}-notation) whenever the
continuation must inspect the return value, as is the case for all
data-dependent future conditions.
...
\paragraph{Good program: \lstinline{v <- takeMVar 1; putMVar v}}
...
\[
  ...
  \pre  = \univ.
\]
Binding \lstinline{putMVar 1}
(whose $\pre = \previously{\mathtt{takeMVar}(1)}$,
$\post = \mathtt{putMVar}(1)$):
\begin{itemize}
  \item \textit{Precondition.}
    \[
      \post(\mathtt{takeMVar}(1)) \setminus \pre(\mathtt{putMVar}(1))
      = \mathtt{takeMVar}(1) \setminus \previously{\mathtt{takeMVar}(1)}
      = \univ,
    \]
  ...
  \item \textit{Future.}
    ...
    intersecting with
    $\fut(\mathtt{putMVar}(1)) = \noUntil{\mathtt{putMVar}(1)}{\mathtt{takeMVar}(1)}$
    gives $\noUntil{\mathtt{putMVar}(1)}{\mathtt{takeMVar}(1)}$.\ \checkmark

  \item[] (Nullable: the program may end here; no double-put obligation is pending.)
\end{itemize}

\paragraph{Bad program: \lstinline{putMVar 1} alone}

\lstinline{pure ()} has $\post = \varepsilon$, so
\[
  \varepsilon \setminus \previously{\mathtt{takeMVar}(1)}
  = \previously{\mathtt{takeMVar}(1)},
\]
which is not nullable (the empty trace contains no \lstinline{takeMVar} event).
Hence
\[
  \pre = \univ \wedge \previously{\mathtt{takeMVar}(1)}
       = \previously{\mathtt{takeMVar}(1)},
\]
which is not $\univ$, so the precondition is violated.\ \texttimes

\paragraph{Bad program: \lstinline{missingPut}
  (\lstinline{takeMVar 1 >> takeMVar 2 >> putMVar 1})}

The future of \lstinline{takeMVar 2} is $\finally{\mathtt{putMVar}(2)}$,
which is not discharged by \lstinline{putMVar 1}.
The residual future retains $\finally{\mathtt{putMVar}(2)}$, naming the
blocked \lstinline{MVar} precisely.\ \texttimes

\paragraph{Bad program: \lstinline{doublePutImmediate}
  (\lstinline{v <- takeMVar 1; putMVar v; putMVar v})}

After the first \lstinline{putMVar 1}, the future is
\[
  \noUntil{\mathtt{putMVar}(1)}{\mathtt{takeMVar}(1)}.
\]
The second \lstinline{putMVar 1} has $\post = \mathtt{putMVar}(1)$;
subtracting it from the formula above yields $\varnothing$,
so the future collapses to $\varnothing$.\ \texttimes
...
applies the following RE-specific laws, which exploit the complement
constructor:
...
\paragraph{What are the monad laws?}
...
--- extracting sub-expressions, fusing steps, or reordering
\lstinline{do}-blocks --- without changing the observable semantics.

\paragraph{Why verify Pledge?}
\lstinline{Pledge} extends the standard monad with three extra fields:
\lstinline{pre}, \lstinline{post}, and \lstinline{future}.
The bind formula for each field (Equations~\ref{eq:future-bind}
and~\ref{eq:pre-bind}) is non-trivial, involving subtraction and
conjunction.
Without verification, these fields could interact with \lstinline{>>=}
in ways that violate the laws, making composed specifications sensitive to
accidental parenthesisation.
Confirming lawfulness ensures that a programmer who writes
\lstinline{takeMVar v >>= putMVar} and one who writes the equivalent
in two steps obtain identical \lstinline{pre}, \lstinline{post}, and
\lstinline{future} annotations.

\paragraph{What the laws check and the setup.}
Because \lstinline{future} has type \lstinline{a -> eff}, the laws are
stated in terms of \lstinline{evalFuture}, defined as
...
assumption that \lstinline{Composable} forms a monoid under \lstinline{<>}
with unit \lstinline{empty}, and that \lstinline{universe} is the unit for
\lstinline{(/\\)}.
The \lstinline{pre} field satisfies the Hoare-rule property
(Equation~\ref{eq:pre-bind}) rather than strict monad-law equality, because
it accumulates \emph{residual} precondition obligations rather than tracking
a symmetric state; we return to this at the end of the section.

\paragraph{Left identity.} \lstinline{pure x >>= f = f x}.

\lstinline{pure x} contributes no postcondition ($\post = \varepsilon$)
and no future obligation ($\mathtt{future} = \lambda\,\_.\ \univ$), so
binding it should leave \lstinline{f x} entirely unchanged.
...
The key step is $r \setminus \univ = \varepsilon$: subtracting the universal
obligation discharges everything, leaving the empty residual $\varepsilon$.
This follows from the base case $\univ \setminus r = \varepsilon$ of
\lstinline{reSubtraction}.

\paragraph{Right identity.} \lstinline{e >>= pure = e}.

\lstinline{pure(ret e)} has $\post = \varepsilon$ and
$\widehat{\fut} = \univ$, so it contributes nothing to the composed
specification, and the result should equal $e$ exactly.
...
The key step is $\varepsilon \setminus r = r$
(base case of \lstinline{reSubtraction}: consuming an empty prefix leaves the
obligation unchanged), and $r \wedge \univ = r$ (unit for $\wedge$).
...
\]
Intuitively, successive postconditions peel away layers of the accumulated
future obligation from the inside out, and the order of peeling is determined
by the order of events, not by parenthesisation.

\paragraph{Precondition.}
The \lstinline{pre} field does not satisfy the monad laws as equalities,
and this is intentional.
The standard monad laws require that both sides of each equation are
\emph{identical}; \lstinline{pre} instead satisfies the Hoare-rule property
(Equation~\eqref{eq:pre-bind}):
\[
  \pre(e \mathbin{>\!\!>\!\!=} f)
  \;=\; \pre(e) \;\wedge\; \bigl(\post(e) \setminus \pre(e_2)\bigr).
\]
The residual $\post(e) \setminus \pre(e_2)$ captures exactly which part
of $e_2$'s precondition $e$'s postcondition has not yet discharged.
When $\post(e)$ fully covers $\pre(e_2)$, the residual is $\varepsilon$
and the combined \lstinline{pre} collapses to $\pre(e)$ --- the standard
Hoare sequencing rule.
When coverage is absent, the residual is $\varnothing$, immediately
flagging a precondition violation.
This asymmetry is a feature: \lstinline{pre} accumulates
\emph{unmet obligations} forward through the chain, precisely recording
which precondition remains unsatisfied and why.
...
Atkey~\cite{atkey2009} introduces parameterised monads, written
...
execution by instrumenting or replaying a concrete run.
Pledge takes a \emph{dynamic-checking} approach: future conditions are
obligation values propagated through the \lstinline{Pledge} structure,
and the resulting residuals can be inspected at any point in the program
(e.g.\ as assertions in \lstinline{main}, as shown in
Section~\ref{sec:discussion}).
Crucially, no concrete execution is required to expose a violation; the
obligation algebra surfaces contradictions as soon as the relevant
annotations are composed---prior to any effectful run.
Runtime monitoring instruments an \emph{actual} execution and reports
violations only after the fact; Pledge propagates and checks obligations
in the specification layer, independently of any running effects.
...
or \lstinline{mask} --- so that the obligation is both expressed and
guaranteed --- remains future work.
