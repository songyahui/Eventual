...
Consider three scenarios drawn directly from the Haskell ecosystem.

\emph{TLS lifecycle.}
The \lstinline{tls} library establishes a secure connection with
\lstinline{handshake :: Context -> IO ()}, after which the caller may
exchange data freely.
The library documentation is explicit: \lstinline{bye} should be called
before closing the handle; otherwise the TLS session may not be
resumable.
A post-condition on \lstinline{handshake} can record that the handshake
succeeded, but it cannot enforce that \lstinline{bye :: Context -> IO ()}
is called before the \lstinline{Context} is discarded, because the
discharge point lies in an arbitrary future context, possibly deep inside
an exception handler or a different module.
The \lstinline{bracket} pattern sidesteps this by lexically scoping the
context, but bracket cannot express the richer obligation that
\emph{certain messages must be exchanged} between handshake and bye,
nor can it handle the case where the context must be passed across
function boundaries.

\emph{MVar discipline.}
\lstinline{takeMVar :: MVar a -> IO a} removes the value from a shared
variable, leaving it empty.
The caller is obliged to restore it with \lstinline{putMVar} on every
exit path, including those reached via asynchronous exceptions.
A survey of Hackage found that roughly half of libraries using
\lstinline{bracket} or \lstinline{onException} contain subtle
async-exception bugs at precisely this pattern: the obligation is
introduced by \lstinline{takeMVar} but must be manually discharged on
every continuation, including those the author did not write.
A post-condition on \lstinline{takeMVar} records that the variable is
now empty; it cannot require that a future \lstinline{putMVar} on the
\emph{same} variable will occur.

\emph{Database savepoints.}
\lstinline{persistent}'s \lstinline{transactionSave} commits the current
transaction and opens a fresh one, introducing an obligation that the new
inner transaction must itself eventually be committed or rolled back
before \lstinline{runSqlConn} exits.
Pre/post-conditions on \lstinline{transactionSave} can record the
transition; they cannot carry forward the obligation that an appropriate
closing call must follow.

In each case the obstacle is the same: the \emph{postcondition} of the
initiating operation cannot name what comes next, because the
continuation is unknown at specification time.
The operation can only record a debt that the remainder of the program
must honour.

\paragraph{Existing remedies and their limits.}
The Haskell community has developed several partial solutions.
\lstinline{bracket} lexically scopes cleanup: it handles \emph{resource}
obligations but not \emph{protocol} obligations that must propagate beyond
the lexical scope.
\lstinline{ResourceT}~\cite{snoyman2011resourcet} and
\lstinline{Acquire}/\lstinline{Managed} thread cleanup actions through
every bind, but model only the cleanup half of an obligation and discharge
it uniformly at scope exit, not by application-specific dischargers such
as \lstinline{commit}, \lstinline{bye}, or protocol-specific events.
Linear Haskell~\cite{bernardy2018linear} enforces ``consume exactly
once'' for individual values, ruling out dropped handles, but cannot
express branching discharge obligations such as ``either
\lstinline{commit} or \lstinline{rollback},'' nor quantitative
obligations such as ``complete with probability at least $p$''.
...
\lstinline{QualifiedDo} to recover \lstinline{do}-notation, produce
opaque type errors, and are confined to a single domain per design.
...
whatever handle was actually returned via the lambda parameter \lstinline{r}.
\lstinline{putMVar} asserts that \lstinline{putMVar(v)} must not recur
until \lstinline{takeMVar(v)} is called again, preventing double-put while
permitting re-acquisition.
...
The \lstinline{tls} library's lifecycle obligation is representative of
a broad class of protocol obligations in Haskell: a successful
\lstinline{handshake} must always be paired with a subsequent
\lstinline{bye}, even across exception handlers.
...
\lstinline{takeMVar} returns the handle of the taken \lstinline{MVar}; its
data-dependent future condition uses the lambda parameter \lstinline{r} so
the obligation always tracks the \emph{returned} handle, not a call-site
constant.
\lstinline{putMVar} carries $\noUntil{\mathtt{putMVar}(v)}{\mathtt{takeMVar}(v)}$
as its future, forbidding a second \lstinline{putMVar(v)} unless
\lstinline{takeMVar(v)} resets the guard first.
...
Atkey~\cite{atkey2009} introduces \emph{parameterised notions of
computation}: a parameterised monad $T(S_1, S_2, A)$ represents a
computation that starts in state $S_1$, ends in state $S_2$, and yields
a value of type $A$; sequencing is defined only when the post-state of
the first matches the pre-state of the second.
This is the categorical foundation underlying indexed monads and was
explicitly motivated by Hoare-logic-style pre/post state tracking.
Katsumata~\cite{katsumata2014} generalises this to graded monads, where
the grade is drawn from a monoid rather than a fixed two-index.
Orchard and Wu~\cite{orchard2014} show that effect systems and session
types are instances of the same parameterised-monad structure.
Pledge is related but orthogonal: rather than indexing computations by
their effect \emph{type} at the type level, we carry an explicit
\emph{temporal obligation} at the value level as the
\lstinline{future :: a -> eff} field.
This sacrifices the compile-time guarantees of type-level indexing in
exchange for compatibility with the standard \lstinline{Monad} class,
\lstinline{do}-notation, and the full \lstinline{mtl} transformer stack.
...
\lstinline{bracket} (Control.Exception) lexically scopes cleanup actions
but cannot express obligations that must propagate beyond the lexical
scope of a single function.
\lstinline{ResourceT}~\cite{snoyman2011resourcet} was designed precisely
to escape lexical scope: it threads a finalizer queue through every bind,
guaranteeing cleanup when the \lstinline{ResourceT} block exits.
The \lstinline{Acquire} and \lstinline{Managed} types (Hackage) generalise
this to an applicative/monadic interface for composing resource lifetimes.
All three mechanisms handle the \emph{cleanup} half of a resource
obligation but cannot express \emph{protocol} obligations
(``after handshake, the context must receive bye'') or
\emph{quantitative} obligations (``complete with probability $\geq p$''),
which require an explicit future-condition annotation to propagate across
bind steps.

\paragraph{Linear types.}
Linear Haskell~\cite{bernardy2018linear} introduces multiplicity
annotations on function arrows, enforced by GHC's type checker since
version 9.0.
The \lstinline{linear-base} library~\cite{linearbase2021} uses this
to provide a \lstinline{RIO} monad whose file handles are linear: the type
system statically guarantees that every opened handle is consumed exactly
once, catching dropped handles at compile time.
Linear types are a powerful complement to future conditions: they enforce
\emph{structural} obligations (use exactly once) whereas future conditions
enforce \emph{temporal} obligations (use in a specified order, or with a
specified probability).
A hybrid design combining linear types for uniqueness and
\lstinline{Pledge} for temporal ordering could achieve stronger guarantees
than either alone.

\paragraph{Effect systems and algebraic effect handlers.}
The \lstinline{Pledge} monad is related to algebraic effect
handlers~\cite{pretnar2015}, which decompose effectful computations into
an operation set and a separate handler that interprets them.
In the handler framework, resource lifecycle obligations are typically
encoded in the handler's state machine rather than in the effect type
itself.
Polysemy's \lstinline{Scoped} effect (Tweag, 2022) is a notable
Haskell instance: it attaches a resource creation and cleanup action to a
lexical scope, threaded through the effect row.
Pledge takes the complementary route: obligations are values in the
\lstinline{Pledge} monad's annotation fields, composable across any
handler.
...
The \lstinline{sessiontypes} Haskell library~\cite{sessiontypes2017}
realises this via an indexed monad \lstinline{STTerm m s s' a}, where the
indices $s$ and $s'$ are the pre- and post-session states; the Haskell
type checker then rejects programs that violate the protocol order.
This gives compile-time guarantees that Pledge does not: Pledge checks
obligations by inspecting normalised annotation values, not by type
inference.
Pledge is more lightweight in exchange: it operates within the standard
\lstinline{Monad} class, admits quantitative and heap-ownership
obligations that session types do not address, and is applicable to
domains beyond message-passing communication.
...
at the cost of requiring language or type-checker extensions.
Brady's \lstinline{ST} monad encoding in Idris~\cite{brady2013idris}
demonstrates that Hoare-logic-style pre/post-state indices can be
expressed in a dependently typed language with full compile-time
enforcement.
Pledge achieves comparable expressiveness at the library level, relying
solely on standard Haskell type classes and requiring no modifications to
GHC, at the cost of moving checking to the annotation-inspection phase
rather than type-inference time.
...
spec :: Pledge RE ()
spec = do { v <- takeMVar 1; putMVar v }

-- Implementation (real IO via effectful)
impl :: (IOE :> es) => Eff es ()
impl = do { v <- liftIO takeMVarReal; liftIO (putMVarReal v) }
