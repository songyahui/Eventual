...
``a taken \lstinline{MVar} must eventually be put back,'' or ``a submitted
...
\emph{particular} \lstinline{MVar} that was taken must eventually be put back.
...
A call to \lstinline{takeMVar} is not erroneous in isolation, but it creates
an \emph{obligation}: the taken \lstinline{MVar} must eventually be put back,
exactly once, with no intervening double-put.
...
Nor is the phenomenon confined to concurrency protocols.
...
naming, for instance, which specific \lstinline{MVar} was never put back, or which transaction
was left open.
...
For example, \lstinline{takeMVar} can declare that \emph{whichever}
\lstinline{MVar} handle it returns must eventually be put back: not a
fixed handle determined at specification time, but the one actually taken
at runtime.
...
\begin{itemize}
  \item \emph{(Trace protocol, $\RE$)}
        ...
        \lstinline{takeMVar}/\lstinline{putMVar} pairing, transaction
