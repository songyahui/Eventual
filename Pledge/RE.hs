...
-- | Language equality: test both REs on all words over 'testAlph' up to length 3.
-- Structural 'Eq' is insufficient because e.g. @Seq (Seq a b) c /= Seq a (Seq b c)@
-- even though the two accept the same language; 'langEq' gives the correct notion.
...
-- | QuickCheck: @(·)@ is associative: @(x · y) · z = x · (y · z)@.
