...
empty; the caller must restore it with \lstinline{putMVar} on every exit
path including async-exception paths.
...
\[
  \noUntil{\mathtt{putMVar}(1)}{\mathtt{takeMVar}(1)}.
\]
The second \lstinline{putMVar 1} contributes $\post = \mathtt{putMVar}(1)$;
subtracting $\mathtt{putMVar}(1)$ from this formula yields $\varnothing$,
so the combined future collapses to $\varnothing$.\ \texttimes
