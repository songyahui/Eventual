...
import qualified Future as F
import Future (RE, normalize, universe, finally, previously, noUntil)
...
-- ── 3. Shadow type ────────────────────────────────────────────────────────
-- A Shadow pairs the pure RE specification (F.Effectful RE a, checked
...
data Shadow a = Shadow
    { spec :: F.Effectful RE a         -- pure spec, FutureCond checks this
...
-- ── 4. Spec primitives ────────────────────────────────────────────────────
-- For each Eff operation, an F.Effectful RE value carrying the RE contract.

specOpen :: FilePath -> F.Effectful RE ()
specOpen path = F.Effectful
    { F.ret    = ()
    , F.pre    = universe
...
specRead :: FilePath -> F.Effectful RE String
specRead path = F.Effectful
...
specClose :: FilePath -> F.Effectful RE ()
specClose path = F.Effectful
    { F.ret    = ()
    , F.pre    = F.Or (F.Single (F.Atom "open" (F.List [F.Str path])))
                      (F.Single (F.Atom "read" (F.List [F.Str path])))
