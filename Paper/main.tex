...
that impose \emph{trace obligations} on the remainder of a program's execution.
Future conditions are carried as a \emph{data-dependent} field
$\mathtt{future} :: a \to \mathit{eff}$ in the \lstinline{Pledge} monad
alongside pre- and post-conditions, so that the future obligation of an 
operation can depend on its own return value, e.g.,
a returned file handle must be eventually closed.
When operations are composed via monadic bind, future conditions are
applied to the current return value and propagated automatically
using a subtraction operator; pre- and post-conditions compose
in the Hoare-rule style.
...
\lstinline{empty} (for concatenation) and \lstinline{universe} (for conjunction).
We present four instances of the framework: trace protocols via regular expressions ($\RE$);
trace protocols enriched with arithmetic bounds ($\GRE$);
quantitative obligations via semiring-weighted regular expressions ($\WRE$);
and heap-ownership obligations via separation logic ($\SL$).
All four instances share a uniform monadic bind formula, demonstrating
that \emph{Pledge} captures trace-based, arithmetic, quantitative, and
...
a post-condition describes what holds once it returns. Together they characterise the behaviour of a single operation in isolation; yet many correctness properties span
multiple calls with arbitrary computation in between:

\begin{itemize}
  \item \emph{(Trace protocol, $\RE$)}
        Every \lstinline{malloc(a)} must eventually be followed by
        exactly one \lstinline{free(a)} (no double-free), and every
        \lstinline{beginTx} must eventually reach \lstinline{commit}
        or \lstinline{rollback}.
  \item \emph{(Arithmetic bounds, $\GRE$)}
        A counter must not exceed its declared maximum before being reset;
        both the ordering constraint and the numeric bound must hold simultaneously.
  \item \emph{(Quantitative obligation, $\WRE$)}
        A submitted task must complete with probability at least $p$,
        or be scheduled within a minimum-cost number of steps.
  \item \emph{(Heap ownership, $\SL$)}
        Heap cells must be owned before access and released after use;
        separating conjunction ensures that each cell is owned by
        exactly one caller at a time.
\end{itemize}

These are \emph{future conditions}: obligations that an operation imposes
on the \emph{remainder} of the program upon completion.
Pre- and post-conditions are insufficient to express them.
...
Both are local to a single operation boundary, discharged at the point of the call.
...
and fully discharged only when the requisite sequence of operations has been completed.
Should a computation terminate with an undischarged future condition,
the residual annotation identifies precisely what remains outstanding.

We propose the \lstinline{Pledge} monad as a lightweight, compositional
mechanism for specifying such obligations.
A value of type \lstinline{Pledge eff a} represents a computation
that returns a value of type $a$ and carries three annotations:
a precondition $\mathtt{pre} :: \mathit{eff}$, a postcondition $\mathtt{post} :: \mathit{eff}$,
and a data-dependent future condition $\mathtt{future} :: a \to \mathit{eff}$ that
expresses what the remaining execution must satisfy as a function of
the return value of type $a$.
When two computations are sequenced via monadic bind, the future
condition of the first is \emph{partially discharged} by the
postcondition of the continuation using the \lstinline{subtraction}
operator of the \lstinline{Composable} algebra; the residual is then
conjoined with the continuation's own future condition.
...
\begin{enumerate}
  \item We define the \lstinline{Pledge} monad (\S\ref{sec:effectful}),
        ...
        $\mathtt{pre} :: \mathit{eff}$, $\mathtt{post} :: \mathit{eff}$, and $\mathtt{future} :: a \to \mathit{eff}$
  ...
  \item We instantiate the algebra to extended regular expressions over
        ...
        subtraction (\S\ref{sec:re}).
        We also show that LTL\textsubscript{f} operators embed directly
        as RE terms (\S\ref{sec:ltl}).
  \item We extend the RE instance to a \emph{guarded} form ($\GRE$,
        \S\ref{sec:guardedre-instance}), pairing each RE with a Presburger
        arithmetic predicate over program variables.  The predicate is
  ...
  \item We provide a fourth, independent instantiation to separation-logic
        ...
        conjunction serves as sequential composition, magic wand as
        subtraction, and Presburger arithmetic provides
...
monadic bind analogously to our \lstinline{Pledge} monad.
